...
def log(pLog):
    ...
    #본인이 로그 기록하고 싶은 경로
    fname = "C:\\Users\\Administrator\\OneDrive - 연세대학교 (Yonsei University)\\융합캡스톤디자인(암호화폐 자동매매)\\AutoTrading\\log_" + str(datetime.datetime.today().strftime('%Y%m%d')) + ".txt"
...
def sell_crypto_currency(ticker, currency):
    for item in upbit.get_balances():
        if item['currency'] == currency:
            unit = item['balance'] #해당 코인 개수 (KRW이면 원화)
            ret = upbit.sell_market_order(ticker, unit - 1)
...
while True:
    try:
        ...
        if criteria < now < criteria + datetime.timedelta(seconds=10):
            ...
            sell_crypto_currency("KRW-DOGE", "DOGE")
    except:
    ...
    try:
        ############## 단계 3. 매수 시도 ###############
        if datetime.datetime.now().second % 30 == 0: #30초에 한번씩 현재 가격로그 기록
            log("current_price_DOGE : " + str(current_price_DOGE) + " / target_price_DOGE : " + str(target_price_DOGE))
        if current_price_DOGE > target_price_DOGE:
            log("!!!!!!!!!!!!!!!buying start!!!!!!!!!!!!!!!!!!!")
            log("current_price_DOGE : " + str(current_price_DOGE) + " / target_price_DOGE : " + str(target_price_DOGE))
